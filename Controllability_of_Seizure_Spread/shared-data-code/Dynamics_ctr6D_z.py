...
class Dynamics_ctr6D_z:
    

    def __init__(self,model,Numeric_method,Network,CTRL):
        ...
        self.mHist=0
        self.Ktr=CTRL["Ktr"]
        self.X_str=CTRL["X_str"]
        self.Ktr_D=CTRL["Ktr_D"]
        self.Ktr_lag=CTRL["Ktr_lag"]
        self.Ktr_W=CTRL["Ktr_W"]
        self.t_Ktr_on=0
        self.t_Ktr_off=0
        self.Ktr_flag=False
        self.Ktr_01=0
        ...
        self.Y=np.zeros([self.M_down,self.N,3])
        ...
        self.z_onset=np.zeros(self.N)
        self.z_offset=np.zeros(self.N)
        self.termination_time=np.ones(self.N)*self.M*self.h
        self.seizure_01=np.zeros(self.N).astype(int)
        self.refractory_onoff=np.zeros(self.N).astype(int)
        self.seizure_onsets=np.zeros(self.N)
        self.seizure_offsets=np.zeros(self.N)
        self.KTR_offsets=np.zeros(self.N)
        self.q=0
        self.x0_ref=self.x00[0]
        self.zmax=np.zeros(self.N)
    ...
    def One_step(self):
        ...
        if self.Numeric_Method=="Runge_Kutta_2" and self.model_dim==6:
            ...
            if(np.mod(self.t,self.d_down)==0):
                ...
                self.Y[t_down,:,0]=self.X[3,self.t2,:]-self.X[0,self.t2,:]
                self.Y[t_down,:,1]=self.X[0,self.t2,:]
                self.Y[t_down,:,2]=self.X[2,self.t2,:]
    ...
    def run(self,t_EZ_onset,EZ_x0):
        ...
        while (self.t<self.M):
            ...
            self.Onset_Offset_Detector(t_EZ_onset)
            ...
            if(self.Ktr_flag==True):
                if(self.seizure_01[self.EZs[0]]>0):
                    self.t_Ktr_on=self.seizure_onsets[self.EZs[0]]+self.Ktr_lag
                    self.t_Ktr_off=self.t_Ktr_on+self.Ktr_D
                    if(self.t*self.h>self.t_Ktr_on and self.t*self.h<self.t_Ktr_off):
                        self.Ktr_01=1
                    else:
                        self.Ktr_01=0
            ...
            if(self.t==self.M-1 and self.q==0):
                ...
                self.q=1
                if(np.size(a)>0):
                    self.Y=np.append( self.Y , np.zeros([int(0.5*self.M_down)+1,self.N,3]) ,axis=0)
    ...
    def F(self):
        if self.model_name=="epileptor" and self.model_dim==6:
            ...
            self.dX[0,:] = self.XX[1] - f1(self.XX[0],self.XX[3],self.XX[2]) - self.XX[2] + self.I1 + self.Ktr_01 * self.Ktr_W[0] * self.Ktr * (self.X_str[0]-self.XX[0]) 
            self.dX[1,:] = 1.0 - 5.0 * self.XX[0]**2 - self.XX[1] + self.Ktr_01 * self.Ktr_W[1] * self.Ktr * (self.X_str[1]-self.XX[1])
            self.dX[2,:]= (self.r) * ( 4.0* (self.XX[0]-self.x0) - self.XX[2] - self.hist )  + self.Ktr_01 * self.Ktr_W[2] * self.Ktr * (self.X_str[2]-self.XX[2]) 
            self.dX[3,:]= self.XX[3] - self.XX[4] - self.XX[3]**3 + self.I2 + 0.002 * self.XX[5] - 0.3 * ( self.XX[2] -3.5 ) + self.Ktr_01 * self.Ktr_W[3] * self.Ktr * (self.X_str[3]-self.XX[3])
            self.dX[4,:]= (1.0/self.tau2) * ( f2( self.XX[3] ) - self.XX[4]  ) + self.Ktr_01 * self.Ktr_W[4] * self.Ktr * (self.X_str[4]-self.XX[4])
            self.dX[5,:]= self.XX[0] -  self.gamma*self.XX[5]+ self.Ktr_01 * self.Ktr_W[5] * self.Ktr * (self.X_str[5]-self.XX[5])
    ...
    def Set_Initial_Condition(self,Y0):
        ...
        self.Y[0:int(self.t0/self.d_down)+1,:,0]=Y0[3]-Y0[1]
        self.Y[0:int(self.t0/self.d_down)+1,:,1]=Y0[0]
        self.Y[0:int(self.t0/self.d_down)+1,:,2]=Y0[2]
    ...
    def Onset_Offset_Detector(self,t_EZ_onset):
        e=1
        th_of=-80.0
        th_on=-120.0
        if(self.t>self.t0+int(t_EZ_onset/self.h)):
            ...
            for i in a[0]:
                if(self.X[5,self.t1,i]>th_on):
                    ...
                    self.z_onset[i]=self.X[2,self.t1,i]
            ...
            for i in a[0]:
                if(self.X[5,self.t1,i]<th_of+e and self.X[5,self.t1,i]>th_of-e):
                    ...
                    if(self.X[5,t3,i]-self.X[5,self.t1,i]>0.01*e):
                        ...
                        self.seizure_offsets[i]=self.t*self.h
                        self.z_offset[i]=self.X[2,self.t1,i]
                        # self.Ktr[i]=0
                
                if(self.zmax[i]<self.X[2,self.t1,i]):
                    self.zmax[i]=self.X[2,self.t1,i]
            
            
            if(self.Ktr_flag==True and self.KTR_of_flag==0):

                # a=np.where(self.refractory_onoff==1)
                # for i in a[0]:
                #     if( self.Ktr[i]==1 and np.absolute( self.X[2,self.t1,i] - self.X_str[2,i] ) <0.05 ):
                #         self.Ktr[i]=0
                #         self.KTR_offsets[i]=self.t*self.h
                #         self.refractory_onoff[i]=2


                a=np.where(self.seizure_01>0)[0]
                b=np.where(self.refractory_onoff>0)[0]
                if(np.size(a)>0 and np.size(a) == np.size(b) ):
                    self.KTR_of_flag=1
                    self.Ktr_W[:]=0
                    c=np.where(self.Ktr==1)
                    for i in c[0]: 
                        self.KTR_offsets[i]=self.t*self.h
    ...
    def x0_to_refractory(self,t_EZ_onset):
        a=np.where(self.seizure_01>1)
        for i in a[0]:
            if(self.refractory_onoff[i]==0):
                if(self.X[2,self.t1,i]<self.X_str[2,i]+0.1*(self.zmax[i]-self.X_str[2,i])):
                    self.x0[i]=self.x0_ref
                    self.refractory_onoff[i]=1
...
'''
   ...
   # Old copy of the refractory condition
    def x0_to_refractory(self,t_EZ_onset):
        e=1
        th_of=-80.0
        th_on=-120.0
