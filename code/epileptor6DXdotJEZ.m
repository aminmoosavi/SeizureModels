function [Xdot,J] = epileptor6DXdotJEZ(X,tau,W,x0,EZ,x1_EZ)
% 6d-epileptor -- An et al. (2019), p. 16
...
x1x1=zeros(N,N); y1x1=zeros(N,N); zx1=-(1/tau0) * WW; x2x1=zeros(N,N);
...
for i=1:N
    ...
    x1z(i,i)=-1-df1dz;
    zx1(i,i)=(1/tau0) * ( 4 + sum(WW(i,:))+WiEZ );
    x1x1(i,i)=-df1dx1;
    x1x2(i,i)=df1dx2;
    ...
    dz(i,1)=(1/tau0) * ( 4*(x1(i,1)-x0(i,1)) - z(i,1) - WW(i,:)*x1 ...
        + sum( WW(i,:) ) * x1(i,1) - WiEZ*( x1_EZ - x1(i,1) ) );
    
    dx2(i,1)= -y2(i,1) + x2(i,1) - x2(i,1)^3 +I2 +0.002*g(i,1) ...
        - 0.3*(z(i,1)-3.5);
